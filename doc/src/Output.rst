...
|SExtractor++| computes only what is required to produce the output catalog and diagnostics.
While the original |SExtractor| would automatically trigger the measurement processes depending on the columns the user would like to be included in the catalog, |SExtractor++| adopts a more coarse-grained and modular approach through the concept of *properties*.
...
The ``--list-output-properties`` option prints out the complete list of available properties known to |SExtractor++|.
...
The complete list of columns and properties currently known to |SExtractor++| is given below.
