...
The safest way to find objects is indeed using ``--weight-type=background`` which determines the
...
disk space is contributed through temporary files. Depending on the situation (data size, available disk space)
...
A solution is to switch off the multi-threading of all lower-level libraries by setting the following environment variables
