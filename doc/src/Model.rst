...
In |SourceXtractor++|, any of the model parameters :math:`q_j` may be a constant parameter, a free parameter, or a dependent parameter.
...
|SourceXtractor++| comes with two pre-defined free parameters for easily initializing positions (:func:`~config.model_fitting.get_pos_parameters()`) and fluxes (:func:`~config.model_fitting.get_flux_parameter()`):
