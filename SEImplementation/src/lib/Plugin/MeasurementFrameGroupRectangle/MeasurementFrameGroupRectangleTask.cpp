...
void MeasurementFrameGroupRectangleTask::computeProperties(SourceGroupInterface& group) const {
  ...
  const auto& measurement_frame_info = group.begin()->getProperty<MeasurementFrameInfo>(m_instance);
  ...
  if (bad_coordinates || max_coord.m_x < 0 || max_coord.m_y < 0 || min_coord.m_x >= measurement_frame_info.getWidth() ||
      min_coord.m_y >= measurement_frame_info.getHeight()) {
    group.setIndexedProperty<MeasurementFrameGroupRectangle>(m_instance, bad_coordinates);
  }
