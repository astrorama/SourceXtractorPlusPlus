...
std::shared_ptr<Task> DetectionFrameImagesTaskFactory::createTask(const PropertyId& property_id) const {

  if (property_id == PropertyId::create<DetectionFrameImages>()) {
    return std::make_shared<DetectionFrameImagesTask>();
  } else {
    return nullptr;
  }
}
