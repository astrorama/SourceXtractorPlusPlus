...
void FlexibleModelFittingSersicModel::addForSource(const FlexibleModelFittingParameterManager& manager,
                          const SourceInterface& source,
                          std::vector<ModelFitting::PointModel>& point_models,
                          std::vector<ModelFitting::TransformedModel>& extended_models,
                          std::tuple<double, double, double, double> jacobian,
  ...
  sersic_component.emplace_back(new SersicModelComponent(make_unique<OldSharp>(), i0, *manager.getParameter(source, m_n), k));
