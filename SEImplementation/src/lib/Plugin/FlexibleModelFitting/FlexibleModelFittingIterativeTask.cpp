...
void FlexibleModelFittingIterativeTask::computeProperties(SourceGroupInterface& group) const {
  ...
  for (auto& source : group) {
    ...
    for (auto frame : m_frames) {
      ...
      if (isFrameValid(source, frame_index)) {
        auto stamp_rect = getFittingRect(source, frame_index);
        initial_state.fitting_aeras_x.push_back(stamp_rect.getWidth());
        initial_state.fitting_aeras_y.push_back(stamp_rect.getHeight());
      } else {
        initial_state.fitting_aeras_x.push_back(-1.f);
        initial_state.fitting_aeras_y.push_back(-1.f);
      }
    }
  ...
  for (auto& source : group) {
    ...
    source.setProperty<FlexibleModelFitting>(source_state.iterations, source_state.stop_reason,
        ...
        meta_iterations, source_state.fitting_aeras_x, source_state.fitting_aeras_y);
