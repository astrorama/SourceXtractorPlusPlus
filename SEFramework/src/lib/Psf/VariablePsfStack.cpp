...
void VariablePsfStack::setup (std::shared_ptr<CCfits::FITS> pFits)
  {
	...
	if (phdu.axes () != 0)
	  {
	    ...
	    << "The primary HDU is not empty! File: " << pFits->name ();
	  }
	...
	if (psf_data.axes () != 2)
	  {
	    throw Elements::Exception ()
	    << "The first HDU is not an image! File: " << pFits->name ();
	  }
	...
	if (m_psf_size % 2 == 0)
	  {
	    throw Elements::Exception ()
	    << "PSF kernel must have odd size, but has: " << m_psf_size;
	  }
	...
	try
	{
	    ...
	    position_data.column ("GRIDX", false).read (m_gridx_values, 0,
							m_nrows);
	    position_data.column ("GRIDY", false).read (m_gridy_values, 0,
							m_nrows);
	}
	catch (CCfits::Table::NoSuchColumn)
	{
	    position_data.column ("X_CENTER", false).read (m_gridx_values, 0,
							   m_nrows);
	    position_data.column ("Y_CENTER", false).read (m_gridy_values, 0,
							   m_nrows);
	}
    ...
    {
	throw Elements::Exception () << "Error loading stacked PSF file: "
	    << e.message ();
    }
  }
...
void VariablePsfStack::selfTest(){
    ...
    const auto x_grid_minmax = std::minmax_element (begin (m_gridx_values),
						    end (m_gridx_values));
    const auto y_grid_minmax = std::minmax_element (begin (m_gridy_values),
						    end (m_gridy_values));
...
std::shared_ptr<VectorImage<SeFloat>> VariablePsfStack::getPsf(const std::vector<double> &values) const {
  ...
  // give some feedback
  stack_logger.debug() << "Distance: "<< sqrt(min_distance) << " (" << values[0] << "," << values[1] << ")<-->(" << m_x_values[index_min_distance] << "," << m_y_values[index_min_distance] << ") index: " << index_min_distance;
